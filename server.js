...
// ========== CLIENT ANTI-TAMPER / ANTI-LOGGER ==========
const CLIENT_PROTECTION_LUA = `--[[ QyrexApi Client Protection ]]
local function __qyrexAntiDebug()
  local _g, _d = getfenv, debug
  local _i = _d and _d.getinfo
  if _g then
    local ok, e = pcall(_g)
    if ok and e then
  ...
  local _h = 0
  pcall(function()
    game:GetService("RunService").Heartbeat:Connect(function() _h = _h + 1 end)
  end)
  task.delay(0.12, function()
    if _h < 1 then error("protected", 0) end
  end)
end
pcall(__qyrexAntiDebug)

local function __qyrexInitAntiLogger()
  local _rawget, _rawset = rawget, rawset
  local _pcall, _tostring = pcall, tostring
  local _debug = debug
  local _debug_getinfo = _debug and _debug.getinfo
  local _getmetatable = getmetatable
  local _getfenv = getfenv
  local _Instance_new = Instance and Instance.new
  local _game = game
  local _game_GetService = _game and _game.GetService
  local _task = task
  local _task_wait = _task and _task.wait
  local _task_spawn = _task and _task.spawn

  local function IsCClosure(fn)
    if not fn or not _debug_getinfo then return true end
    local ok, info = _pcall(_debug_getinfo, fn)
    if not ok or not info then return true end
    return info.what == "C" and info.source == "=[C]"
  end

  local DetectionFlags = {}
  local function FlagDetection(reason)
    DetectionFlags[#DetectionFlags + 1] = { time = tick(), reason = reason }
  end

  local function CheckFunctionIntegrity(fn, expectedName)
    if not fn then return true end
    if not IsCClosure(fn) then return false, expectedName .. " hooked" end
    return true
  end

  local function CheckMetatableIntegrity(tbl, name)
    if not tbl then return true end
    local mt = _getmetatable(tbl)
    if mt and (rawget(mt, "__index") or rawget(mt, "__newindex")) then
      return false, name .. " meta"
    end
    return true
  end

  local HeartbeatCanary = 0
  pcall(function()
    local RunService = _game_GetService(_game, "RunService")
    RunService.Heartbeat:Connect(function()
      HeartbeatCanary = HeartbeatCanary + 1
    end)
  end)

  local genv_ref
  pcall(function() if getgenv then genv_ref = getgenv() end end)

  local function VerifyEnvironment()
    if _getfenv then
      local ok, err = CheckFunctionIntegrity(_getfenv, "getfenv")
      if not ok then FlagDetection(err) end
    end
    if _debug_getinfo then
      local ok, err = CheckFunctionIntegrity(_debug_getinfo, "debug.getinfo")
      if not ok then FlagDetection(err) end
    end
    local ok, err = CheckMetatableIntegrity(_G, "_G")
    if not ok then FlagDetection(err) end
    if genv_ref then
      local ok2, err2 = CheckMetatableIntegrity(genv_ref, "getgenv")
      if not ok2 then FlagDetection(err2) end
    end
    if _Instance_new then
      local ok3, err3 = CheckFunctionIntegrity(_Instance_new, "Instance.new")
      if not ok3 then FlagDetection(err3) end
    end
    if _game_GetService then
      local ok4, err4 = CheckFunctionIntegrity(_game_GetService, "GetService")
      if not ok4 then FlagDetection(err4) end
    end
    local current = HeartbeatCanary
    if _task_wait then _task_wait(0.03) end
    if HeartbeatCanary == current then FlagDetection("Heartbeat stalled") end
  ...
  if _task_spawn then
    _task_spawn(function()
      while true do
        pcall(VerifyEnvironment)
        if _task_wait then _task_wait(0.15 + math.random() * 0.1) end
      end
    end)
  else
    pcall(VerifyEnvironment)
  ...
  local SafeEnv = {
    IsSecure = function() return #DetectionFlags == 0 end,
    GetFlags = function() return DetectionFlags end,
    ForceVerify = VerifyEnvironment,
  }
  pcall(function()
    if getgenv then getgenv().AntiLogger = SafeEnv end
  end)
  return SafeEnv
end

local __AntiLogger = __qyrexInitAntiLogger()
if __AntiLogger and not __AntiLogger.IsSecure() then
  error("protected", 0)
end
`;

function wrapWithClientProtection(source) {
  return CLIENT_PROTECTION_LUA + "\n" + String(source || "");
}

function lightXorShield(scriptCode) {
  const src = Buffer.from(String(scriptCode || ''), 'utf8');
  if (src.length === 0 || src.length > 1500000) return String(scriptCode || '');
  const key = crypto.randomInt(1, 255);
  const xorKey = crypto.randomInt(1, 255);
  const shift = crypto.randomInt(1, 50);
  ...
  return [
    ...
    '  b=bit32 and bit32.bxor(bit32.bxor(b,(i%256)),bit32.bxor(_x,_k)) or (b~((i%256)~_x~_k))',
    '  _r[i]=string.char(b)',
    'end',
...
async function serveRealScript(req, res, scriptId) {
  ...
  // Anti-debug + Anti-logger + XOR shield (servidor)
  try {
    payload = wrapWithClientProtection(payload);
    payload = lightXorShield(payload);
  } catch (e) {
    console.error('client protection wrap', e);
  }
